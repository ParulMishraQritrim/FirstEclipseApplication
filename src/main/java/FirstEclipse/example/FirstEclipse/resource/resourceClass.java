...
public class resourceClass
{
   ...
   { 
		...
		if (resp > 0) {
			...
			respStatus.setCode(pfmsg.getValue("qivr.inf.id.success.code"));
			respStatus.setMsg("successfully deleted clause!!");
			FEResponseEntity.setStatus(respStatus);
			response = new ResponseEntity<>(FEResponseEntity, HttpStatus.OK);
		} else {
			respStatus.setCode(pfmsg.getValue("qivr.inf.id.invalid.code"));
			respStatus.setMsg(pfmsg.getValue("qivr.activity.id.invalid.msg"));
			FEResponseEntity.setStatus(respStatus);
			response = new ResponseEntity<>(FEResponseEntity, HttpStatus.BAD_REQUEST);
			return response;
		}
